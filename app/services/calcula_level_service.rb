class CalculaLevelService
  ...
  def execute
    ...
    # Abertura de chamado 1 pontos
    pontos = 1
    object_yield = UserBackoffice::Yield.find_or_create_by(submitter_id: @new_ticket&.submitter_id.to_i)
    # Novo ticket
    ...
    if @old_ticket[:status] == 'solved'
      ...
      if object_yield.present?
        tickets_xp_sum = object_yield.submitter.tickets.where.not(id: @new_ticket.id).map(&:xp).sum
