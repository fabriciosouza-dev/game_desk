  ...
  def execute
    ...
    client.tickets.each do |ticket|
      ...
      if ticket_object.present?
        ticket_object.update(atributos_tickets(ticket).except(:ticket_id)) if ticket_object.changed_at.to_s != ticket[:updated_at].to_s
        ...
        comments = []
        new_comments = []
        ticket_object = Ticket.create(atributos_tickets(ticket))
